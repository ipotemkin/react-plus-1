...
export const StateButton: FC<StateButtonProps> = ({
  text = 'StateButton', buttonState = 'normal', size = 'm'
}) => {
  const [state] = useState(String(buttonState)) // без String при передаче значения в cnStateButton вылетает ошибка
  const [btnSize] = useState(String(size)) // без String при передаче значения в cnStateButton вылетает ошибка
  
  // вот это хотелсь бы исключить и опираться на стили в css
  let iconColor = 'black'
  if (state.includes('rased') || state.includes('action')) iconColor = 'white'
  ...
  return (
    <>
      <button className={cnStateButton({ type: state, size: btnSize })}>
